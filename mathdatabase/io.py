...
def __add_helper(mdb, data):
    ...
    if os.path.exists(datatype):
        ...
        mdb.get_file_number()[datatype] += 1
        mdb.get_file_number()["remaining-" + datatype] += 1

        filename = "file" + str(mdb.get_file_number()[datatype]) + ".txt"

        for attribute in mdb.get_attributes():

            dir_attribute = os.path.join(mdb.get_current_path(), datatype, attribute)
        ...
        dir_log = os.path.join(mdb.get_current_path(), "log.txt")
        with open(dir_log, 'w') as outfile:
            json.dump(mdb.get_file_number(), outfile)

        # return index sha key
        return mdb.get_sha(os.path.join(mdb.get_current_path(), datatype, "index"), filename)
...
def remove_instance(mdb, data):
    ...
    if os.path.exists(datatype):
        ...
        dir_index = os.path.join(mdb.get_current_path(), datatype, "index")
        ...
        for filename in os.listdir(dir_index):
            ...
            if input_sha == mdb.get_sha(dir_index, filename):

                for attribute in mdb.get_attributes():

                    path = os.path.join(mdb.get_current_path(), datatype, attribute)
                ...
                mdb.get_file_number()["remaining-" + datatype] -= 1
                with open(os.path.join(mdb.get_current_path(), "log.txt"), 'w') as outfile:
                    json.dump(mdb.get_file_number(), outfile)
...
def retrieve_instance(mdb, data):
    ...
    if not os.path.exists(datatype):
        ...
        for filename in os.listdir(dir_index):
            ...
            if input_sha == mdb.get_sha(dir_index, filename):
                for attribute in mdb.get_attributes():
                    file_path = os.path.join(mdb.get_current_path(), datatype, attribute, filename)
...
def __update_file(content, path, commit_message, filename):
        ...
        for filename in os.listdir(dir_index):
            ...
            if input_sha == mdb.get_sha(dir_index, filename):
                for attribute in mdb.get_attributes():
                    ...
                    __update_file(data[attribute], dir_attribute, data["commit"], filename)
                status = 1
                break
